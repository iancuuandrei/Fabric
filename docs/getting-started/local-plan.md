...
Run `harness --root PATH init`, then edit `PATH/harness.toml`. Its default check
is Go tests; use checks appropriate to your repository. Checks are declared and
bound in this milestone, but not executed yet.

Run `harness --root PATH doctor` to validate configuration and commit identity.
...
An actor label records operator input, not cryptographic authentication.
...
State lives under `.harness/runs/`. Ignore `.harness/` in the target repository.
